...
State-of-the-art low-power neural network accelerators have demonstrated significant improvements in both energy efficiency and computational performance using 8-bit \gls{fp} arithmetic for training and inference. 

\cite{wu2020phoenix} proposes the Phoenix architecture implementing 8-bit \gls{fp} quantization. Key findings suggest that this method incurs less error than its fixed-point counterpart. Normalization prior to quantization aids in further error reduction. Phoenix outperforms other state-of-the-art accelerators when benchmarked with AlexNet and VGG16.

Another work, \cite{park2021neural}, presents an 8-bit \gls{fp} neural network training processor that leverages shared exponent bias (FP8-SEB) for non-sparse neural networks. This implementation uses multiple-way fused multiply-add (FMA) trees for maintaining high numerical precision and reducing energy consumption, demonstrating improved efficiency against conventional neural network training processors.

\cite{venkataramani2021rapid} introduces RaPiD, an accelerator that supports various precisions. Notably, the accelerator can handle 16 and 8-bit \gls{fp} computations, as well as 4 and 2-bit fixed-point calculations. The chip, fabricated in 7nm EUV technology, achieves peak performance metrics that are competitive for both inference and training tasks.

Lastly, \cite{venkataramanaiah202228nm} discusses an 8-bit \gls{fp} tensor core-based CNN training processor. This processor incorporates highly parallel tensor cores, ensuring high utilization throughout various phases of the training process. With the integration of dynamic output activation sparsity and other efficiency-enhancing features, this 28nm prototype chip showcases impressive improvements across different training tasks.



\subsection{Training Techniques with 8-bit Floating-Point Quantization}
The quest for efficient neural network training has spawned a significant interest in reduced precision computations, especially using 8-bit \gls{fp} quantization. One of the early forays into this domain was the use of 8-bit \gls{fp} for training the LeNeT-5 model for handwritten digit classification, achieving a commendable 97.10\% accuracy while reducing space complexity by 75\% \cite{gallus2018handwritten}. A similar venture demonstrated the feasibility of training deep neural networks using 8-bit \gls{fp} numbers, introducing novel techniques such as chunk-based accumulation, which holds the potential for up to 4 times improved throughput in future hardware platforms \cite{wang2018training}.

A significant milestone is the hybrid 8-bit \gls{fp} (HFP8) format, which was specifically crafted for deep learning applications ranging from image classification to language processing \cite{sun2019hybrid}. This was further optimized in a mixed precision training approach that utilized an enhanced loss scaling method and a stochastic rounding technique to address gradient noise, surprisingly reporting even better accuracies than full precision \cite{mellempudi2019mixed}.

Taking a different tack, adaptive floating-point (AFP) post-training quantization emerged as a promising approach, offering compression rates and inference efficiency, sidelining the need for computationally intensive de-quantization \cite{liu2021improving}. The overarching advantages of 8-bit \gls{fp} over fixed-point representation were further evaluated in a comprehensive study, showcasing its promise for a range of deep learning applications without compromising on accuracy \cite{noune20228}.
