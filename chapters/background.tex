...
Neural networks are computational models inspired by biological neural networks in the human brain. These algorithms are designed to extract patterns, interpret data, and approximate complex functions. Their architecture comprises interconnected nodes (neurons) organized into layers. Each connection possesses a weight, which is adjusted during learning. The primary components include:
...
\begin{itemize}
	\item \textbf{Sigmoid}: 
The sigmoid function, denoted as \( \sigma(z) \), is especially used in binary classification tasks. Mathematically, the sigmoid function is defined as:
...
However, the sigmoid function is not without its drawbacks. For very large or very small values of \( z \), the function becomes saturated, leading to small gradients and, consequently, slow convergence during training. This phenomenon is often referred to as the "vanishing gradient" problem.
...
\gls{sgd} is an iterative optimization algorithm used to minimize an objective function that is defined as a sum of differentiable functions. It's particularly well-suited for problems with a large number of training samples.
...
A \gls{mlp} is a type of feedforward artificial neural network, consisting of multiple layers of interconnected neurons. The architecture of an MLP can be defined as:
...
\begin{enumerate}
	...
	\item One or more \textbf{hidden layers} that transform the input data. Each neuron in a hidden layer computes a weighted sum of its inputs, applies a bias term, and then undergoes an activation function.
...
While quantization techniques continue to evolve, making fixed-point deployment feasible for many neural networks, it remains essential to rigorously evaluate fixed-point representations in the above contexts to ensure desired performance and accuracy.
...
The dot-product, a foundational operation in neural networks, involves a series of multiplications followed by their summation. This directly maps onto a series of \gls{mac} operations. The \gls{mac} operation is foundational in digital signal processing and, it is central to the architecture and efficiency of neural network accelerators. Formally, the \gls{mac} operation can be expressed as:
\begin{equation}
